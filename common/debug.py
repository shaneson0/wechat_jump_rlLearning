...
def save_debug_screenshot2(ts, im, piece_x, piece_y, board_x, board_y, new_piece_x, new_piece_y):
    make_debug_dir(screenshot_backup_dir)
    draw = ImageDraw.Draw(im)
    # draw.line((piece_x, piece_y) + (board_x, board_y), fill=2, width=3)

    draw.line((new_piece_x, 0, new_piece_x, im.size[1]), fill=(255, 0, 0))
    draw.line((0, new_piece_y, im.size[0], new_piece_y), fill=(255, 0, 0))
    ...
    # draw.ellipse((piece_x, piece_y , piece_x , piece_y ), fill=(255, 0, 0))
    # draw.ellipse((board_x , board_y , board_x , board_y ), fill=(0, 0, 255))

    draw.ellipse((new_piece_x - 10, new_piece_y - 10, new_piece_x + 10, new_piece_y + 10), fill=(255, 0, 0))
...
def computing_error(last_press_time, target_board_x, target_board_y, last_piece_x, last_piece_y, temp_piece_x,
    ...
    target_distance = math.sqrt( (target_board_y - last_piece_y) ** 2)
    actual_distance = math.sqrt((temp_piece_y - last_piece_y) ** 2)

    # print(round(target_distance), round(jump_error_value), round(actual_distance), round(last_press_time))
    return target_distance, actual_distance
    ''''# 将结果采集进学习字典
